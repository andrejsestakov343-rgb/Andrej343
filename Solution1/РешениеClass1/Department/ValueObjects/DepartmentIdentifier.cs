    ...
    public sealed record DepartmentIdentifier
    {
        ...
        public static DepartmentIdentifier Create(string value)
        {
            ...
            if (value.Length >  MaxLength)
                throw new ArgumentException($"Идентификатор подразделения не может быть превышать {MaxLength} символов.", nameof(value));
            ...
            if (!Regex.IsMatch(value, Pattern))
                throw new ArgumentException("Идентификатор подразделения должен содержать только строчные латинские буквы, цифры и дефисы.", nameof (value));
